...
This is the archived v1 project. Run the commands below from this `v1/`
directory so relative output paths point to the archived project.

This project compares two COMSOL simulations of a partially welded pipe wall:
`data/models/weld_defect_v1.mph` has a lack-of-fusion defect, and
`data/models/weld_nodefect_v1.mph` is the reference. The receiver exports
still name these simulations `*_v4.mph`; the project owner confirmed that
the current `v1` archives are the matching models. Exported receiver traces are in
